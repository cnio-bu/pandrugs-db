    ...
    for index, row in filei[['drug_name', 'gene_name']].drop_duplicates().iterrows():
        ...
        if len(selection) > 0:
            ...
            if len(selection.loc[selection['response'] == 'sensitivity']) > 0:
                response.append('sensitivity')
                alteration_sen = '; '.join(list(set(selection.loc[selection['response'] == 'sensitivity']['alteration'].tolist())))
            if len(selection.loc[selection['response'] == 'resistance']) > 0:
                response.append('resistance')
                alteration_res = '; '.join(list(set(selection.loc[selection['response'] == 'resistance']['alteration'].tolist())))
            ...
            if len(response) > 0:
                ...
                fileo = fileo.append({'gene_name' : row['gene_name'], 'drug_name' : row['drug_name'], 'source' : source, 'response' : ' / '.join(response), 'alteration' : alteration} , ignore_index=True)
...
def process_oncoKB():
    ...
    for index, row in drug_gene.iterrows():
        alteration_sen = []
        alteration_res = []
        oncokb_subset = oncokb_select.loc[(oncokb_select['Drugs'] == row['Drugs']) & (oncokb_select['Gene'] == row['Gene'])]
        ...
        if len(alteration_sen) > 0 and len(alteration_res) > 0:
            response = 'sensitivity / resistance'
            variation = '; '.join(sorted(list(set(alteration_sen))))+' / '+'; '.join(sorted(list(set(alteration_res))))
        elif len(alteration_sen) > 0:
            response = 'sensitivity'
